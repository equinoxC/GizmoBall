...
public class CrashHandler implements CrashInterface {
    ...
    private boolean isCrashedTragle(int ballx,int bally,int temp_x,int temp_y,int radius){
        ...
        if (hab<=radius || hac<=radius || hbc<=radius) return true;
        return false;
    }
    ...
    private boolean isCrashedCircle(int ballx,int bally,int temp_x,int temp_y,int radius){
        if ((ballx-temp_x-15)*(ballx-temp_x-15)+(bally-temp_y-15)*(bally-temp_y-15)<=(radius+15)*(radius+15)) return true;
        return false;
    }
    ...
    public String isCrashed() {
        ...
        if(x!=0 && y!=0 && win.getPoint(x,y).isPiece()){
            ...
            int ball_x = x + radius;
            int ball_y = y + radius;
            if(win.getPoint(x,y).getPiece().getName() == "redRectangular") return "redRectangular";
